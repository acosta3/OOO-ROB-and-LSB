...
namespace ns3 {
    ...
    CpuCoreGenerator::CpuCoreGenerator(CpuFIFO* associatedCpuFIFO, int robSize, int lsbSize, int ipc) {
        // default
        m_coreId         = 1;
        m_cpuCycle       = 1;
        ...
        m_dt             = 1;
        m_clkSkew        = 0;
        m_cpuMemReq      = new CpuFIFO::ReqMsg();
        m_cpuMemReq->done[0] = false;
        m_cpuMemReq->done[1] = false;
        m_cpuMemResp     = CpuFIFO::RespMsg();
        m_cpuInstruction = new CpuFIFO::ReqMsg();
        m_cpuInstruction->done[0] = false;
        m_cpuInstruction->done[1] = true;
        ...
        m_readTrace       = false;
        m_ROB            = ROB(robSize, ipc);
        m_LSB            = LSB(associatedCpuFIFO, lsbSize);
        m_numHits        = 0;
    ...
    void CpuCoreGenerator::ProcessTxBuf() {
        ...
        //std::cerr << "HELLO\n";
          ...
          if (m_newSampleRdy == false) {
            ...
            if (getline(m_bmTrace,fline)){
             ...
             std::string num_compute_ins   = fline.substr(0, pos1);
             std::string addr              = fline.substr(pos1 + 1, pos2 - pos1 - 1);
             std::string type              = fline.substr(pos2 + 1);

             m_currentInstructionNum = (uint64_t) strtol(num_compute_ins.c_str(), NULL, 10);
             m_cpuMemReq = new CpuFIFO::ReqMsg();
             // convert hex string address to decimal 
             m_cpuMemReq->addr = (uint64_t) strtol(addr.c_str(), NULL, 10);
             
             //std::cout << addr << " " << m_cpuMemReq->addr << "\n";
             //exit(1);
             ...
             //std::cerr << "MESSAGE ID: " << IdGenerator::nextReqId() << "\n";
             m_cpuMemReq->reqCoreId = m_coreId;
	     m_cpuMemReq->done[0] = false;
	     m_cpuMemReq->done[1] = false;
	     m_cpuMemReq->sent = false;
             m_cpuReqCnt++;
             }
            } 
            if (m_readTrace){
            //std::cerr << "HELLO\n";
              while (m_currentInstructionNum > 0 && m_ROB.canAccept()){
                m_cpuInstruction = new CpuFIFO::ReqMsg();
                m_cpuInstruction->type = CpuFIFO::REQTYPE::COMPUTE;
                m_cpuInstruction->addr = 0;
		m_cpuInstruction->done[0] = false;
		m_cpuInstruction->done[1] = true;
                m_ROB.allocate(m_cpuInstruction);
                //std::cerr << m_currentInstructionNum << "\n";
                m_currentInstructionNum--;
              }
              if (m_currentInstructionNum == 0){
               //std::cerr << "MESSAGE ID: " << m_cpuMemReq->msgId << "\n";
                if (m_LSB.canAccept() && m_ROB.canAccept()){
                 //std::cerr << "MESSAGE ID: " << m_cpuMemReq->msgId << "\n";
                  m_ROB.allocate(m_cpuMemReq);
                  bool lsb_hit = m_LSB.allocate(m_cpuMemReq);
        ...
        //std::cerr << m_newSampleRdy << " " << m_readTrace << "\n";
        if (m_bmTrace.eof()) {
          m_bmTrace.close();
          m_cpuReqDone = true;
        }           
        ...
        if (!m_cpuFIFO->m_rxFIFO.IsEmpty()) {
          m_cpuMemResp = m_LSB.rxFromCache();
        ...
        if (m_cpuReqDone == true && m_cpuRespCnt >= m_cpuReqCnt) {
          ...
          std::cout << "Number of LSB Hits: " << m_numHits << "\n";
        }
        else {
    ...
    void CpuCoreGenerator::Step(Ptr<CpuCoreGenerator> cpuCoreGenerator) {
        ...
        cpuCoreGenerator->m_LSB.step();
        cpuCoreGenerator->ProcessTxBuf();
        cpuCoreGenerator->ProcessRxBuf();
        //std::cerr << cpuCoreGenerator->m_cpuMemReq->msgId << "\n";
    }
}
